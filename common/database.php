...
function getConnect()
{
    $connect = mysqli_connect( $GLOBALS['DATABASE_HOST'], $GLOBALS['DATABASE_USER_NAME'], $GLOBALS['DATABASE_PASSWORD']) or mysqli_error();
    mysqli_set_charset($connect, 'utf8');
    $db = mysqli_select_db($connect, 'laomao') or exit("数据库在连接时出现问题，可能是设定的数据库不存在。");
    return $connect;
}
...
function select_data($sql)
{
    ...
    $res = mysqli_fetch_array($link,MYSQLI_ASSOC);
    return $res;
}
